...
const parseDT = (dt = null, format) => parseInt(momentDateToString(dt ? moment(dt) : moment(), format));
...
export function extractTime(endTime, startTime = null, isPast = false) {
...
const addDateTime = (num = 1, time = 'd', format = 'DD/MM/YYYY') => momentDateToString(moment().add(num, time), format);
...
export function asDateTime(endTime = moment(), startTime = moment(), isPast = false, isAbs = false) {
  ...
  return { timeLeft, asYears, asMonths, asWeeks, asDays, asHours, asMinutes, asSeconds };
}
...
export const fdtfM1 = (dt, mx = 'm1b') => {
  ...
  if (fm === `HH[h]` && moment(dt).hour() < 10) return momentDateToString(dt, `H[h]`);
...
export const fdtfM3Custom = (dt, m1, m2) => {
  if (moment().diff(moment(dt, 'YYYY-MM-DD'), 'days') > 3) {
    return `${fdtfM1(dt, m1)} ${fdtfM2(dt, m2)}`;
  }
  if (moment().diff(moment(dt, 'YYYY-MM-DD'), 'days') === 3) {
    return `${fdtfM1(dt, m1)} Hôm kìa`;
  }
  if (moment().diff(moment(dt, 'YYYY-MM-DD'), 'days') === 2) {
    return `${fdtfM1(dt, m1)} Hôm kia`;
  }
  if (moment().diff(moment(dt, 'YYYY-MM-DD'), 'days') === 1) {
    return `${fdtfM1(dt, m1)} Hôm qua`;
  }
  if (moment().diff(moment(dt, 'YYYY-MM-DD'), 'days') === 0) {
    return `${fdtfM1(dt, m1)} Hôm nay`;
  }
  return '';
};
...
export const fdtfM7 = dt => momentDateToString(dt, `[${getWeekDay(dt)}, ngày] DD [tháng] MM [năm] YYYY`);
...
export const fdtfK1 = (dt1, dt2, k = 'k1b') => {
  ...
  if (months > 0 && months < 12) return `${months} tháng${!isC1 && days !== 0 ? ` ${days} ngày` : ''}`;
...
export const fdtfK2 = (dt, k = 'k21') => {
  ...
  if (strK === 'k21a' || strK === 'k21b') return `${dtK} ${timeLeft < 0 ? 'trước' : 'sau'}`;
