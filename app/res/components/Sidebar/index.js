...
const Sidebar = ({ isExpand, minWidth, maxWidth, permissionArray, dataMenu, onClickMenu, pathname }) => {
      ...
      if (menuHaveChilds.length !== 0) {
        ...
        for (let x = 0; x < menuHaveChilds.length; x++) {
          if (
            ...
            _.filter(menuHaveChilds[x].child, i => i.path === pathname).length !== 0
          ) {
            return _.filter(menuHaveChilds[x].child, i => i.path === pathname)[0].key;
          }
        }
      }
    }
            ...
            permissionArray !== undefined ? (
              itemMenu.child ? (
                permissionArray.includes(itemMenu.key) ? (
                  <MenuCustom.SubMenu key={itemMenu.key} icon={<IconSider src={itemMenu.icon} />} title={itemMenu.label}>
                    {_.flatMap(itemMenu.child, subItem =>
                      permissionArray.includes(subItem.key) ? (
                        <MenuCustom.Item
                          key={subItem.key}
                          onClick={onClickMenu !== undefined ? () => onClickMenu(subItem) : null}
                        >
                          <DotSubItem />
                          {subItem.label}
                        </MenuCustom.Item>
                ...
                <MenuCustom.Item
                  ...
                  title={null}
                  onClick={onClickMenu !== undefined ? () => onClickMenu(itemMenu) : null}
                >
                  {itemMenu.label}
                </MenuCustom.Item>
              ) : null
